...
export default function DocFlowSaaS() {
  ...
  const [folders, setFolders] = useState<FolderItem[]>([
    { id: 'f1', name: 'Equipamentos & Máquinas', description: 'Ativos Fixos e Maquinaria (SNC 43 / 611 - IVA 100%)', color: 'emerald' },
    { id: 'f2', name: 'Manutenção & Peças', description: 'Assistência técnica e componentes (SNC 6222 - IVA 100%)', color: 'blue' },
    { id: 'f3', name: 'Consumíveis & Produtos', description: 'Stock de armazém e consumíveis (SNC 611 - IVA 100%)', color: 'amber' },
    { id: 'f4', name: 'Instalações & Energia', description: 'Água, eletricidade e comunicações (SNC 6221/6226 - Débito Direto)', color: 'purple' },
    { id: 'f5', name: 'Alimentação & Refeições', description: 'Refeições de representação (SNC 6251 - IVA 0% Art.21 CIVA)', color: 'red' },
    { id: 'f6', name: 'Combustíveis & Frotas', description: 'Gasóleo e viaturas da empresa (SNC 6242 - IVA 50% Dedutível)', color: 'orange' },
    { id: 'f7', name: 'Fornecedores Espanha / UE', description: 'Aquisições Intracomunitárias (SNC 611/62 - Autoliquidação 0%)', color: 'cyan' },
    { id: 'f8', name: 'Proformas & Orçamentos', description: 'Documentos pendentes de fatura definitiva', color: 'slate' }
  ]);
  ...
  const [archivedDocs, setArchivedDocs] = useState<InvoiceData[]>([
    {
      ...
      items: [
        ...
        { code: '0432430025', description: 'TRAVESSA OVAL INOX 25x19x2CM ESP 0,7MM', quantity: 1, unitPrice: 2.00, total: 1.50 },
        { code: '04324300302', description: 'TRAVESSA OVAL INOX 30x20x2CM ESP 0.7MM', quantity: 1, unitPrice: 2.60, total: 1.95 },
  ...
  const classifySNC = (category: string, total: number) => {
    ...
    if (cat.includes('refeição') || cat.includes('alimenta')) return { snc: 'SNC 6251 - Refeições (0% IVA Art.21 CIVA)', vatRate: 0 };
    if (cat.includes('combust') || cat.includes('gasóleo')) return { snc: 'SNC 6242 - Combustíveis (50% IVA Dedutível)', vatRate: 50 };
      ...
      if (parsed) {
        ...
        if (matchedSup && matchedSup.daysToDue > 0) {
          try {
            ...
            d.setDate(d.getDate() + matchedSup.daysToDue);
  ...
  const handleSaveSupplier = () => {
    if (!supName.trim() || !supNif.trim()) {
      alert('Preencha o Nome e o NIF do fornecedor.');
      return;
    }
  ...
  const handleDeleteSupplier = (id: string) => {
    if (confirm('Tem a certeza de que deseja eliminar esta regra de fornecedor?')) {
  ...
  const filteredSuppliers = suppliers.filter(s => {
    const matchSearch = s.name.toLowerCase().includes(supplierSearchFilter.toLowerCase()) || s.nif.toLowerCase().includes(supplierSearchFilter.toLowerCase());
    if (supplierCountryFilter === 'ALL') return matchSearch;
    return matchSearch && s.country === supplierCountryFilter;
  });
  ...
  const NavItem = ({ id, icon: Icon, label, badge }: any) => {
    ...
    return (
      <button onClick={() => { setActiveTab(id); setSelectedFolderDetail(null); setMobileMenuOpen(false); }} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 group ${isActive ? 'bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.05)]' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'}`}>
    ...
    <div className="flex h-screen bg-[#030712] text-slate-200 overflow-hidden font-sans selection:bg-emerald-500/30">
      ...
      <aside className="hidden md:flex flex-col w-72 border-r border-slate-800/60 bg-[#0B0F19]/80 backdrop-blur-xl relative z-20">
        <div className="p-6 flex items-center gap-3 border-b border-slate-800/60">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold">Suite Contabilidade & IA</p>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto relative pt-16 md:pt-0 pb-20 md:pb-0 scroll-smooth">
        ...
        <div className="p-4 md:p-8 max-w-6xl mx-auto relative z-10 min-h-full">
              ...
              <div className="bg-[#0B0F19]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-white">Selecione onde guardar</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <select value={selectedFolder} onChange={(e) => setSelectedFolder(e.target.value)} className="bg-slate-900 border border-slate-700/50 rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:ring-2 focus:ring-emerald-500/50 cursor-pointer flex-1 sm:flex-none">
                    {folders.map(f => <option key={f.id} value={f.name}>{f.name}</option>)}
                  </select>
                  <button onClick={() => setShowNewFolderModal(true)} className="bg-slate-800 hover:bg-slate-700 text-emerald-400 border border-slate-700 px-3 py-2.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 shrink-0">
              ...
              {invoice && (
                <div className="bg-[#0B0F19]/80 backdrop-blur-xl border border-white/5 rounded-3xl p-6 md:p-8 space-y-6 shadow-2xl animate-in slide-in-from-bottom-6 duration-300">
                  ...
                  <button onClick={handleSave} disabled={loading || saveSuccess} className={`w-full py-4 rounded-xl font-bold text-white shadow-xl transition-all flex items-center justify-center gap-2 text-sm ${saveSuccess ? 'bg-emerald-500' : 'bg-emerald-600 hover:bg-emerald-500'}`}>
                    {saveSuccess ? <><CheckCircle2 className="w-5 h-5" /> Documento Arquivado & Sincronizado!</> : <><Send className="w-5 h-5" /> Confirmar & Arquivar Fatura</>}
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Controlo de Débitos Diretos e Transferências Bancárias</p>
                </div>
          ...
          {activeTab === 'documents' && (
            <div className="space-y-6 animate-in fade-in duration-500">
              <h2 className="text-2xl font-bold text-white tracking-tight">Arquivo Digital Geral</h2>
              ...
              <div className="bg-[#0B0F19]/80 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                <div className="p-4 border-b border-white/5 flex flex-wrap gap-4 items-center justify-between">
                  ...
                  <div className="relative w-full sm:w-72">
                    ...
                    <input type="text" placeholder="Pesquisar faturas..." onChange={(e)=>setSearchFilter(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl pl-9 pr-4 py-2 text-xs text-white outline-none focus:ring-1 focus:ring-emerald-500" />
                  </div>
                </div>
                        ...
                        <tr key={doc.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="p-4 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button onClick={() => togglePaymentStatus(doc.id, doc.paymentMethod === 'Débito Direto')} className={`px-2.5 py-1 rounded-lg text-xs font-bold ${doc.paymentStatus === 'PAID' ? 'bg-white/5 text-slate-400' : 'bg-emerald-600 text-white'}`}>
              ...
              {selectedFolderDetail ? (
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
                    <div className="flex items-center gap-3">
                      <button onClick={() => setSelectedFolderDetail(null)} className="p-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl transition-all">
                        <ArrowLeft className="w-5 h-5" />
                      </button>
                      <div>
                        <div className="flex items-center gap-2 text-xs text-slate-400 font-medium">
                          <span>Pastas</span> <ChevronRight className="w-3.5 h-3.5" /> <span className="text-emerald-400">{selectedFolderDetail}</span>
                        </div>
                        <h2 className="text-2xl font-bold text-white tracking-tight mt-0.5">{selectedFolderDetail}</h2>
                      </div>
                    </div>
                  ...
                  <div className="bg-[#0B0F19]/80 border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                    <div className="p-4 border-b border-white/5 flex items-center justify-between">
                      ...
                      <div className="relative w-64">
                        ...
                        <input type="text" placeholder="Filtrar nesta pasta..." value={folderSearchFilter} onChange={(e)=>setFolderSearchFilter(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white outline-none" />
                      </div>
                    </div>

                    {archivedDocs.filter(d => d.category === selectedFolderDetail).length === 0 ? (
                      ...
                      <table className="w-full text-left text-sm text-slate-300">
                        ...
                        <tbody className="divide-y divide-white/5">
                          {archivedDocs.filter(d => d.category === selectedFolderDetail && d.supplierName.toLowerCase().includes(folderSearchFilter.toLowerCase())).map(doc => (
                            <tr key={doc.id} className="hover:bg-white/[0.02] transition-colors">
                              <td className="p-4 px-6 font-mono text-xs">{doc.docDate}</td>
                              ...
                              <td className="p-4 text-center">
                                <button onClick={() => setSelectedDocDetail(doc)} className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg">
                                  <Eye className="w-3.5 h-3.5" />
                                </button>
                              </td>
                            </tr>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between border-b border-white/5 pb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">Clique em qualquer pasta para abrir e ver os documentos arquivados</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {folders.map(f => {
                      ...
                      return (
                        <div 
                          key={f.id} 
                          onClick={() => setSelectedFolderDetail(f.name)}
                          className="bg-[#0B0F19]/80 border border-white/5 hover:border-emerald-500/40 hover:bg-[#0B0F19] cursor-pointer rounded-2xl p-5 shadow-xl transition-all duration-200 flex flex-col justify-between space-y-4 group"
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <h3 className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors leading-snug">{f.name}</h3>
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#0B0F19]/80 p-3 rounded-2xl border border-white/5">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <button onClick={() => setSupplierCountryFilter('PT')} className={`px-3 py-1.5 rounded-xl text-xs font-bold ${supplierCountryFilter === 'PT' ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-500 hover:text-white'}`}>🇵🇹 Portugal</button>
                  <button onClick={() => setSupplierCountryFilter('ES')} className={`px-3 py-1.5 rounded-xl text-xs font-bold ${supplierCountryFilter === 'ES' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-500 hover:text-white'}`}>🇪🇸 Espanha</button>
                  <button onClick={() => setSupplierCountryFilter('UE')} className={`px-3 py-1.5 rounded-xl text-xs font-bold ${supplierCountryFilter === 'UE' ? 'bg-blue-500/20 text-blue-400' : 'text-slate-500 hover:text-white'}`}>🇪🇺 União Europeia</button>
                </div>

                <div className="relative w-full sm:w-64">
                  <Search className="w-3.5 h-3.5 text-slate-500 absolute left-3 top-2.5" />
                  <input type="text" placeholder="Procurar fornecedor..." value={supplierSearchFilter} onChange={(e) => setSupplierSearchFilter(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white outline-none" />
                </div>
              </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
                              ...
                              {sup.country === 'ES' ? '🇪🇸 Espanha' : sup.country === 'UE' ? '🇪🇺 UE' : '🇵🇹 Portugal'}
                            </span>
                            <span className="text-xs font-mono text-slate-400 font-bold">NIF: {sup.nif}</span>
                          </div>
                          <h3 className="text-base font-bold text-white mt-1.5 leading-snug">{sup.name}</h3>
                          {sup.notes && <p className="text-xs text-slate-400 mt-1">{sup.notes}</p>}
                        </div>

                        <div className="flex items-center gap-1 shrink-0">
                          <button onClick={() => handleOpenSupplierModal(sup)} className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg">
                            <Edit3 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDeleteSupplier(sup.id)} className="p-1.5 bg-slate-800 hover:bg-red-900/40 text-red-400 rounded-lg">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-lg w-full space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Building className="w-5 h-5 text-indigo-400" /> {editingSupplier ? 'Editar Fornecedor & Regra' : 'Criar Nova Regra de Fornecedor'}
              </h3>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div>
                <label className="font-bold text-slate-400 uppercase">País de Origem</label>
              ...
              <div className="col-span-2">
                <label className="font-bold text-slate-400 uppercase">Pasta Predefinida</label>
      ...
      {selectedDocDetail && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-2xl w-full space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest bg-emerald-500/10 px-2.5 py-1 rounded-md border border-emerald-500/20">
                  {selectedDocDetail.docType} • {selectedDocDetail.docNumber}
                </span>
                <h3 className="text-base font-bold text-white mt-1.5">{selectedDocDetail.supplierName}</h3>
              </div>
              <button onClick={() => setSelectedDocDetail(null)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            ...
            {selectedDocDetail.items && selectedDocDetail.items.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-400 uppercase">Linhas Discriminadas</h4>
                <div className="rounded-xl border border-slate-800 bg-slate-950 overflow-hidden">
                  <table className="w-full text-left text-xs text-slate-300 font-mono">
                    <thead className="bg-slate-900 text-slate-500 text-[10px] uppercase border-b border-slate-800">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {selectedDocDetail.items.map((it, idx) => (
                        <tr key={idx}>
                          <td className="p-2.5 text-emerald-400">{it.code || '—'}</td>
                          <td className="p-2.5 font-sans text-white">{it.description}</td>
            ...
            <div className="flex justify-end">
              <button onClick={() => setSelectedDocDetail(null)} className="bg-slate-800 hover:bg-slate-700 text-white font-bold px-5 py-2 rounded-xl text-xs">Fechar</button>
            </div>
          </div>
        </div>
      )}
